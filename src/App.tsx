...
function App() {
  ...
  const fetchAndSetRepositories = async ({
    newPage,
    newName,
  }: {
    newPage?: number;
    newName?: Organization["login"];
  }) => {
    setIsLoading(true);
  ...
  const onOrganizationChange = async (newName: string) => {
    ...
    const newOrganisation = organizations.find(
      ({ login }) => login === newName,
    );
    ...
    <main className={`app ${isLoading ? "loading" : ""}`}>
      ...
      {currentOrg && (
        <FormContext.Provider value={{ formState, dispatch }}>
          <>
            <Form currentOrg={currentOrg} onRefetchClick={onRefetchClick} />
